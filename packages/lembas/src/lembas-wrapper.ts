...
export async function lembasWrapper<T = any>(
  generateDataFn: (...args: any[]) => Promise<T>,
...
async function loadLembas<T>(lembasFilename: string, config: LembasConfig): Promise<T | undefined> {
  ...
  if (fs.existsSync(lembasFilename)) {
    ...
    const rawDbDump = lembasFileContent.substring(rawDataStartPosition, lembasFileContent.length);
...
async function saveLembas<T>(lembasFilename: string, dataToCache: T, config: LembasConfig): Promise<void> {
