...
describe(`getLembasConfig`, () => {
  given('fake lembas.json on the file system', () => {
    when('calling getLembasConfig', async () => {
      ...
      then(
        'config should be the same as on the file system with hooks containing the full path',
        () => {
          expect(actualResult.emptyHook).toContain(
            path.normalize('/packages/lembas/fake')
          );
          expect(actualResult.restoreHook).toContain(
            path.normalize('/packages/lembas/fake')
          );
          expect(actualResult.snapshotHook).toContain(
            path.normalize('/packages/lembas/fake')
          );
        }
      );
